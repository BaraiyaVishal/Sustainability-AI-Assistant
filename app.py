...
# ── Page Config ────────────────────────────────────────────────
st.set_page_config(
    page_title="Sustainability — Energy Intelligence",
    page_icon="🌿",
    layout="wide",
...
When answering, always reference the actual numbers above. Never make up data."""
    ...
    if any(w in ql for w in ["streak","badge","achievement","reward"]):
...
with st.sidebar:
    ...
    <div class="sidebar-logo">
      <h1>🌿 Sustainability AI Assistant</h1>
    ...
    if st.button("⟳  Refresh IoT", use_container_width=True):
...
if ACTIVE == "dashboard":
    ...
    # ── Charts — FIXED: use_container_width=True ──────────────
    sec("Energy Overview")
    ...
    with ch1:
        ...
        fig.add_trace(go.Scatter(
            x=df_p["Date"], y=df_p["kWh"], name="Daily kWh",
            line=dict(color="rgba(0,255,135,0.4)", width=1),
            fill="tozeroy", fillcolor="rgba(0,255,135,0.04)"))
        fig.add_trace(go.Scatter(
            x=df_p["Date"], y=df_p["Roll7"], name="7-Day Avg",
            line=dict(color="#00FF87", width=2)))
        fig.add_hline(
            y=threshold_kwh, line_dash="dash", line_color="#FF5C5C",
            annotation_text="Threshold", annotation_font_color="#FF5C5C",
            annotation_position="top right")
        fig.update_layout(
            **PLOT_LAYOUT, height=320, title="Daily Energy Consumption",
            legend=dict(orientation="h", y=1.12, x=0))
        # ✅ FIXED: use_container_width=True (not width='stretch')
    ...
    with ch2:
        ...
        fig2 = go.Figure(go.Pie(
            labels=last6.index, values=last6.values, hole=0.6,
            marker=dict(colors=["#00FF87","#00CC6A","#00994F","#FFB347","#FF8C42","#FF5C5C"]),
            textfont=dict(size=10)))
        fig2.update_layout(
            **PLOT_LAYOUT, height=320, title="Monthly Split (6mo)",
            showlegend=True,
            legend=dict(font=dict(size=9), x=0, y=-0.1, orientation="h"))
...
# ══════════════════════════════════════════════════════════════
    ...
    st.dataframe(fdisp[["Date","Forecasted_kWh","Lower","Upper","CO₂ (kg)","Cost (₹)","Status"]],
        use_container_width=True,height=min(len(fdisp)*38+50,450))
...
# ══════════════════════════════════════════════════════════════
    ...
    kpi5([
        ...
        ("vs India Avg",    f"{round((INDIA_AVG_KWH-avg_daily)/INDIA_AVG_KWH*100,1)}%","below avg","🇮🇳"),
    ...
    for i, (col, q) in enumerate(zip(pr1+pr2, suggestions)):
        with col:
            if st.button(q, key=f"pill_{i}", use_container_width=True):
                ...
                with st.spinner("Sustainability AI thinking…"):
                    ans = ask_ai(q, ai_ctx)
...
<div style="margin-top:3rem;padding:1.2rem;border-top:1px solid var(--border);
  ...
  <div style="font-family:'Syne',sans-serif;font-weight:700;color:var(--green);font-size:0.9rem">
    🌿 Sustainability AI Assistant v2.0</div>
